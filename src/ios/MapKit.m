...
- (void)createViewWithOptions:(NSDictionary *)options {
    ...
    [self.commandDelegate runInBackground:^{
        // defaults
        float height = ([options objectForKey:@"height"]) ? [[options objectForKey:@"height"] floatValue] : weakSelf.webView.bounds.size.height/2;
        float width = ([options objectForKey:@"width"]) ? [[options objectForKey:@"width"] floatValue] : weakSelf.webView.bounds.size.width;
        float x = ([options objectForKey:@"xOrigin"]) ? [[options objectForKey:@"xOrigin"] floatValue] : weakSelf.webView.bounds.origin.x;
        float y = ([options objectForKey:@"yOrigin"]) ? [[options objectForKey:@"yOrigin"] floatValue] : weakSelf.webView.bounds.origin.y;
        ...
        if(atBottom) {
            y += weakSelf.webView.bounds.size.height - height;
        }
        
        weakSelf.childView = [[UIView alloc] initWithFrame:CGRectMake(x,y,width,height)];
        weakSelf.mapView = [[MKMapView alloc] initWithFrame:CGRectMake(x, y, width, height)];
        weakSelf.mapView.delegate = self;
        weakSelf.mapView.multipleTouchEnabled   = YES;
        weakSelf.mapView.autoresizesSubviews    = YES;
        ...
        MKCoordinateRegion region=[ weakSelf.mapView regionThatFits: MKCoordinateRegionMakeWithDistance(centerCoord,
                                                                                                    diameter*(height / weakSelf.webView.bounds.size.width),
                                                                                                    diameter*(height / weakSelf.webView.bounds.size.width))];
        [weakSelf.mapView setRegion:region animated:YES];
        [weakSelf.childView addSubview: weakSelf.mapView];
        ...
        [imageView2 setFrame:CGRectMake(0,63, weakSelf.childView.bounds.size.width,50)];
        
        [weakSelf.childView addSubview:imageView2];
        
        //puts textfield over the map if the page is newLocation
        if ([page isEqualToString:@"newLocation"]) {
            weakSelf.txtField = [[UITextField alloc] initWithFrame:CGRectMake(10, 125, 300, 40)];
            weakSelf.txtField.borderStyle = UITextBorderStyleRoundedRect;
            weakSelf.txtField.font = [UIFont systemFontOfSize:15];
            weakSelf.txtField.placeholder = @"Nickname";
            weakSelf.txtField.autocorrectionType = UITextAutocorrectionTypeNo;
            weakSelf.txtField.keyboardType = UIKeyboardTypeDefault;
            weakSelf.txtField.returnKeyType = UIReturnKeyDone;
            weakSelf.txtField.clearButtonMode = UITextFieldViewModeWhileEditing;
            weakSelf.txtField.contentVerticalAlignment = UIControlContentVerticalAlignmentCenter;
            weakSelf.txtField.delegate=self;
            [weakSelf.childView addSubview:txtField];
        }
        ...
        [imageView setCenter: weakSelf.mapView.center];
        
        [weakSelf.childView addSubview:imageView];
        
        [ [ [ weakSelf viewController ] view ] addSubview:weakSelf.childView];
    }];
